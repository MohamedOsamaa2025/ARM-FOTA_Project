...
void main(void)
{
	...
	while(1)
	{
		...
		MUSART1_voidReceiveBuffer(u8ReceiveBuffer,150);
		WiFi_voidDisConnectToWiFi(WIFI_SSID, WIFI_PASSWORD);
		...
		for(u8 i =0;i<150;++i)
		{
			if(u8ReceiveBuffer[i] == ':')
			{
				u8RecDataFlag=1;
				u8 j=0;
				while(u8ReceiveBuffer[i+j+1] != 'C' && u8ReceiveBuffer[i+j+2] != 'L' && u8ReceiveBuffer[i+j+3] != 'O' && u8ReceiveBuffer[i+j+4] != 'S' )
				{
