...
void main(void)
{
	...
	while(1)
	{
		...
		else
		{
			...
			{
				if(u8ReceiveBuffer[u16RecDataCounter] == 'C' && u8ReceiveBuffer[u16RecDataCounter+1] == 'L' && u8ReceiveBuffer[u16RecDataCounter+2] == 'O')
				{
					break;
				}
				else
		...
		u8RecDataFlag = 0,u16RecDataCounter = 0;
	}
}
