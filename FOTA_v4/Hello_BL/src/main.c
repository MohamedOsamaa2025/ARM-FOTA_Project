...
u8 u8ReceiveBuffer[1300] = {255};
u8 u8Record[1300] = {255};
u8 u8Parse[100] = {255};
u16 i =0;
u16 j =0;
u8 k=0;
...
void main(void)
{
    ...
    WiFi_voidDisConnectToWiFi(WIFI_SSID, WIFI_PASSWORD);
		...
		MUSART1_voidReceiveBuffer(u8ReceiveBuffer,1300);
		WiFi_voidDisConnectToWiFi(WIFI_SSID, WIFI_PASSWORD);

		// for(u16 i =0;i<2500;++i)
		// {
		// 	if(u8ReceiveBuffer[i] == ':')
		// 	{
		// 		u8RecDataFlag=1;
		// 		i++;
				// for (u16 j = 0; j < 2500 ; j++,i++)
				// {
				// 	if (u8ReceiveBuffer[i] == 255)
				// 	{
				// 		break;
				// 	}
				// 	else
				// 	{
				// 		u8Record[j] = u8ReceiveBuffer[i];
				// 	}
		// 		}
		// 		break;	
		// 	}
		// }
		u8RecDataFlag = 0,i=0,j=0;
		while(u8ReceiveBuffer[i]!=':')
		{
			i++;
		}
		i++;
	...
	// }			
		...
		else
		{
				...
				if (u8BLWriteRequestFlag == 1)
				{
					...
					while (u8Record[j] != 0)
					{
						if(u8Record[j] == 'C' && u8Record[j+1] == 'L' && u8Record[j+2] == 'O')
						{
							break;
						}
						else
						{
							u8Parse[k] = u8Record[j];
							if(u8Parse[k] == '\n')
							{
								/*Parsing the Received Record*/	
								MBL_voidParseRecord(&u8Parse);
								k=0;
							}
							else
							{
